--[[
  ...
  \__|  \__| \_______|\__|  \__| \_______| \_______|\__|                \_/    \_______|$$  ____/  \_______|
   ...
   Library/utils.lua - SystemXVoid/BlankedVoid               
   https://renderintents.xyz
   stupid niggers, fixed by spraying cactus
]]
...
local functions : table = setmetatable({}, {
	__index = function(self, func)
		if disabled then 
			return error(`[Utils Library] - The function handler has been disabled.`)
		end
		if func ~= 'init' and not initiated then 
			return error(`[Utils Library] - Please initiate the function handler using :init() before indexing on it.`)
		end
		local util = rawget(self, func) or getmetatable(self)[func];
		if typeof(util) == 'function' then 
			return util
		end
		return error(`[Utils Library] - Module {func} doesn't exist in the function handler.`)
	end
});
...
self.init = function(self)
	if initiated then
		return error(`[Utils Library] - You can't initiate the function handler more than once.`) 
	end
	initiated = true 
end;
...
self.createutil = function(self, index: string, data): void
	if index:sub(1, 2) == '__' then 
		return error(`[Utils Library] - {index} is a metatable method.`)
	end
...
functions.SetTargetValidation = function(func)
	assert(typeof(func) == 'function', `[❌ Utils Library] function expected for Argument #1, got {typeof(func)}`);
...
self.__newindex = function(self)
	return error(`[Utils Library] - Please refer to using out :creteutil(index: string, data: any) method for adding/modifying data on the function handler.`)
end
